...
deploy_stack $1 $AWS_PROFILE ${APP_CODE}-${ENVIRONMENT}-resources cfn/eks-cluster.yaml cfn/eks-cluster.params ${CLOUDFORMATION_BUCKET} ${APP_CODE} ${CHANGESET_OPTION}  
...
if [[ -z $SECONDARY_REGIONS ]]; then
    ...
    do
        ...
        load_env $r $2
        deploy_stack $r $AWS_PROFILE ${APP_CODE}-${ENVIRONMENT}-resources cfn/eks-cluster.yaml cfn/eks-cluster.params ${CLOUDFORMATION_BUCKET} ${APP_CODE} ${CHANGESET_OPTION}  
    done
fi