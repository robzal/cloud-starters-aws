...
class TransferDealerAdditional(Structure):
    product_family_name = PRODUCT_FAMILY_TRANSFER_DEALER_ADDITIONAL
    supported_product_codes = ['PVRT2060']
    ...
    def parse(self, lines: list) -> list:
        ...
        processed_rows = self.split_content(rows[:-3])
        return processed_rows

    def split_content(self, rows: list) -> list:
        for row in rows:
            row['FullName'] = "{}{}".format(row['ContentToPrint'][2].strip()[:41].strip(), row['ContentToPrint'][3].strip()[:41].strip())
            row['Post_Address_1'] = row['ContentToPrint'][4].strip()[:41]
            row['Post_Address_2'] = row['ContentToPrint'][5].strip()[:41]
            row['Post_Address_3'] = row['ContentToPrint'][6].strip()[:41]
            row['Registration_No'] = row['ContentToPrint'][25].strip()[:6]
            for i, item in enumerate(reversed(row['ContentToPrint']), 1):
                if re.search('Paul Santamaria', item, re.IGNORECASE):
                    del row['ContentToPrint'][-i]
                    break
        return rows
