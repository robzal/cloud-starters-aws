...
class GraduationProbationary(Structure):

    product_family_name = PRODUCT_FAMILY_GRADUATION_PROBATIONARY
    supported_product_codes = ['PNDL1720']

    def parse(self, lines: list):
        headers = {
            Structure.profile_table_mappings['client_id']: [1, 9],
            'Name': [10, 27],
            'Post_Address_1': [37, 27],
            'Post_Address_2': [64, 27],
            'Post_Address_3': [91, 25],
            'Salutation': [116, 20],
            'P1EDate': [136, 10],
            'P2SDate': [146, 10],
            'P2EDate': [156, 10],
        ...
        for i, _line in enumerate(lines):
            ...
            try:
                ...
                # format dates
                row['P1EDate'] = self.get_formatted_date(row['P1EDate'])
                row['P2SDate'] = self.get_formatted_date(row['P2SDate'])
                row['P2EDate'] = self.get_formatted_date(row['P2EDate'])
    ...
    def email_subject(self, row) -> str:
        if row['RecType'] == 'P1P2':
            return 'Graduation from a P1 to a P2 probationary driver licence'
        else:
            return 'Graduation from a P2 to a Full driver licence'

    def history_title(self, row: dict) -> str:
        if row['RecType'] == 'P1P2':
            return 'Congratulations on your graduation from a P1 to a P2 probationary driver licence.'
        else:
            return 'Congratulations on your graduation from a P2 to a Full driver licence.'
