...
class DemeritOptionNotices(Structure):
    product_family_name = PRODUCT_FAMILY_DEMERIT_OPTION_NOTICES
    supported_product_codes = ['PNDL148A', 'PNDL148B']
    ...
    def parse(self, lines: list) -> list:
        ...
        def headers(line):
            headers_01 = {
                ...
                'From_Date': [140, 13],
                'To_Date': [153, 13],
                'Suspension_Mths_Op2': [166, 4],
                'Suspension_Mths_Op1': [170, 4],
                'Commencement_Date': [174, 13],
                'Pin': [187, 4],
                'Filler1': [191, 4],
                'Salutation': [195, 49],
                'EDPPEndDate': [244, 13],
        ...
        for i, _line in enumerate(lines):
            ...
            try:
                if is_type_01(line):
                    ...
                    row['Keep_Date'] = self.get_formatted_date(row['Keep_Date'])
                    row['From_Date'] = self.get_formatted_date(row['From_Date'])
                    row['To_Date'] = self.get_formatted_date(row['To_Date'])
                    row['Commencement_Date'] = self.get_formatted_date(row['Commencement_Date'])
                    row['EDPPEndDate'] = self.get_formatted_date(row['EDPPEndDate'])
                    row['SuspEndDate'] = self.get_formatted_date(row['SuspEndDate'])

                    # trim zeroes
                    row['Demerit_Points'] = self.trim_zeroes(row['Demerit_Points'])
                    row['Suspension_Mths_Op2'] = self.trim_zeroes(row['Suspension_Mths_Op2'])
                    row['Suspension_Mths_Op1'] = self.trim_zeroes(row['Suspension_Mths_Op1'])
                    ...
                    if is_valid_record:
                        offences_dict = {
                            header: line[pad[0] - 1: (pad[0] - 1) + pad[1]].strip() for header, pad in
                            headers(line).items()
                        }

                        # trim zeroes in points and format date infringe
                        for index in range(1, 13):
                            offences_dict['Points' + str(index)] = self.trim_zeroes(
                                offences_dict['Points' + str(index)])
                            offences_dict['DateInfringe' + str(index)] = self.get_formatted_date(
                                offences_dict['DateInfringe' + str(index)])
    ...
    def email_subject(self, row) -> str:
        return 'Demerit points notice'

    def history_title(self, row: dict) -> str:
        return 'Your demerit points limit has been reached. Please review for next steps.'
