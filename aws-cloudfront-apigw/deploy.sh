...
deploy_stack () {
	...
	aws cloudformation deploy \
		...
		$7
        # You can use specific buckets if they are in the same region
		# --s3-bucket $5 \
		# --s3-prefix $6 \
}
...
deploy_stack ${APP_CODE}-cloudfront-apigw ${PRIMARY_REGION} cfn/cloudfront-apigw.yaml cfn/cloudfront-apigw.params ${CLOUDFORMATION_BUCKET} ${APP_CODE} ${CHANGESET_OPTION}  

# SECONDARY_REGIONS
for r in ${SECONDARY_REGIONS//,/ }
do
    load_env $1 $r
    deploy_stack ${APP_CODE}-cloudfront-apigw  ${r} cfn/cloudfront-apigw.yaml cfn/cloudfront-apigw.params ${CLOUDFORMATION_BUCKET} ${APP_CODE} ${CHANGESET_OPTION}  
done
