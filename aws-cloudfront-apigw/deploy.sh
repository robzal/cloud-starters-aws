...
deploy_stack $1 $AWS_PROFILE ${APP_CODE}-cloudfront-apigw cfn/cloudfront-apigw.yaml cfn/cloudfront-apigw.params ${CLOUDFORMATION_BUCKET} ${APP_CODE} ${CHANGESET_OPTION}  
...
if [[ -z $SECONDARY_REGIONS ]]; then
    ...
    do
        load_env $r $1
        deploy_stack $r $AWS_PROFILE ${APP_CODE}-cloudfront-apigw cfn/cloudfront-apigw.yaml cfn/cloudfront-apigw.params ${CLOUDFORMATION_BUCKET} ${APP_CODE} ${CHANGESET_OPTION}  
    done
fi
