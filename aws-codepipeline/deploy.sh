...
deploy_stack $1 $AWS_PROFILE ${APP_CODE}-${ENVIRONMENT}-deployment cfn/template.yaml cfn/template.params ${CLOUDFORMATION_BUCKET} ${APP_CODE} ${CHANGESET_OPTION}  
...
if [[ -z $SECONDARY_REGIONS ]]; then
    ...
    do
        load_env $r $1
        deploy_stack $r $AWS_PROFILE ${APP_CODE}-${ENVIRONMENT}-deployment cfn/template.yaml cfn/template.params ${CLOUDFORMATION_BUCKET} ${APP_CODE} ${CHANGESET_OPTION}  
    done
fi