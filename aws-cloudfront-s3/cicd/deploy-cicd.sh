...
deploy_stack ${APP_CODE}-pipeline ${PRIMARY_REGION} ./pipeline.yaml ./pipeline.params ${CLOUDFORMATION_BUCKET} ${APP_CODE} ${CHANGESET_OPTION}  

# Deployment Role deployed to primary region in any chosen account
deploy_stack ${APP_CODE}-deployment-role ${PRIMARY_REGION} ./pipeline-role.yaml ./pipeline-role.params ${CLOUDFORMATION_BUCKET} ${APP_CODE} ${CHANGESET_OPTION}  
