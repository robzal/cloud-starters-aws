...
deploy_stack ${APP_CODE}-cloudfront-s3 ${PRIMARY_REGION} cfn/cloudfront-s3.yaml cfn/cloudfront-s3.params ${CLOUDFORMATION_BUCKET} ${APP_CODE} ${CHANGESET_OPTION}  
...
do
    ...
    # deploy_stack ${APP_CODE}-cloudfront-s3  ${r} cfn/cloudfront-s3.yaml cfn/cloudfront-s3.params ${CLOUDFORMATION_BUCKET} ${APP_CODE} ${CHANGESET_OPTION}  
done
