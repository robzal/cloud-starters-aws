...
deploy_stack $1 $AWS_PROFILE ${APP_CODE}-${ENVIRONMENT}-custom-resource cfn/custom-resource.yaml cfn/custom-resource.params ${CLOUDFORMATION_BUCKET} ${APP_CODE} ${CHANGESET_OPTION}  
...
if [[ -z $SECONDARY_REGIONS ]]; then
    ...
    do
        load_env $r $1
        deploy_stack $r $AWS_PROFILE ${APP_CODE}-${ENVIRONMENT}-custom-resource cfn/custom-resource.yaml cfn/custom-resource.params ${CLOUDFORMATION_BUCKET} ${APP_CODE} ${CHANGESET_OPTION}  
    done
fi
